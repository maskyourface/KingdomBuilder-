...
## 迭代 10（最终轮 · 用户指令：终止迭代，完善当前方向并交付完成品）

### 分析（debug 回归【便携版 Godot 实测布局与事件行为】/ 逻辑与文档全面核对）
1. **debug 实测**：布局实测确认详情/主菜单/覆灭/事件卡 grow 修复全部生效；发现引导面板
   `grow_vertical` 缺失（整面板出屏——迭代5/6 特性实际不可见）与横幅/toast 容器 `grow_horizontal`
   缺失（从中线向右歪）两处漏网 → 已修；事件卡吞中键松开事件 → `_close_card` 复位 `_panning`；
   覆灭态读取面板死档守卫补齐；apply_effects 正负不对称补齐（wood/stone/clothes/beer 正向）；
   选项置灰扩展到全部资源键；`_paused_by_card` 死变量清理。
2. **逻辑与文档**：README 全面核对——P1×9/P2×9/P3×3 漂移清单（开局资源、覆灭条件、渔屋/货摊表项、
   金币进项、bak 入口矛盾、路线图重复编号、事件系统与引导整节缺失、招满优先级、操作快捷键、
   冬季停产清单、通勤口径、测试断言数、代码结构 13→15）；引导奖励两处边界（毕业跳序、拆建刷奖励）；
   事件状态不入档为已知取舍（读档重摇，README 已声明）。
3. **逻辑正面核对**：事件系统 era/冷却/模态密闭/原子化/双卖货保留线 全部通过。

### 最终落地（全部实施，run_gate GO + logic_test 180/180 实机验证）
- 引导面板 `grow_vertical=BEGIN`（一行修复，引导特性首次真正可见）；横幅/toast 容器 grow BOTH。
- `_close_card` 复位 `_panning`；覆灭态读取面板死档守卫（与 _autosave_if_alive 同口径）。
- apply_effects 正向资源补齐；选项置灰扩展全部资源；`_paused_by_card` 清理。
- 引导奖励：毕业分支放宽（跳序完成可领最后步骤奖励）+ `_tutorial_claimed` 防拆建刷奖 + 差一号修正。
- 暴雨不停渔屋（冬季唯一食物产线）；行商限时代Ⅱ；首张事件卡推迟到第 6 天后（不与引导抢戏）；
  新增冬季限定事件「雪中送炭」（+6 食物）对冲冬季负向事件空缺。
- 事件工匠第二选项加软回报（幸福 +2）形成真抉择；冻结事件排期条件重排。
- README 全面同步（开局资源/覆灭条件/建筑表 24 行/事件系统节/引导节/操作快捷键/代码结构 15 文件/路线图重写）。
- run_gate EXPECTED_SCRIPTS 基线 13→15（含 event_manager/save_manager）。

### 最终验证
- run_gate：**GO**（9 项；--import 零解析错误；smoke/menu/play 全过；logic_test **PASS=180/FAIL=0**）。
- **用户指令：终止迭代循环。本文件自此冻结，后续变更走正常提交流程。**
