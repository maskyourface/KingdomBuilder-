...
func _warn_place_failed(cell: Vector2i) -> void:
	...
	if selected.get("is_road", false):
		place_failed.emit("土路可铺在草地/耕地上（可穿森林），水域、石林与建筑上不行")
		return
...
func _try_place(cell: Vector2i) -> bool:
	if selected.get("is_road", false):
		if grid.can_place_road(cell) and resources.try_spend(selected.get("cost", [])):
...
func _can_place_at(cell: Vector2i) -> bool:
	if selected.get("is_road", false):
		return grid.can_place_road(cell)
